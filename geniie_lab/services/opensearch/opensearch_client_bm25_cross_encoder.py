...
class OpenSearchClientBM25CrossEncoder:
    """
    BM25 retrieval followed by cross-encoder re-ranking.
    """
    ...
    def __init__(
        ...
        model_name = encode_model or "cross-encoder/ms-marco-MiniLM-L-6-v2"
    ...
    def _rerank(self, query: str, hits: List[dict]) -> List[dict]:
        if not hits:
            return []
        pairs = [(query, self._prepare_doc_text(hit.get("_source", {}))) for hit in hits]
        scores = self.model.predict(pairs)
        ranked = sorted(zip(hits, scores), key=lambda x: x[1], reverse=True)
        return [{"hit": hit, "score": float(score)} for hit, score in ranked]
