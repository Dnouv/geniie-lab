#!/usr/bin/env python
"""
Plot average-per-iteration metric groups across topics.

This script generates three figures from the same input logs:
1) LLM metrics: CumRecall, CumPrecision, Jaccard
2) Retriever metrics: CumRecall@100, CumPrecision@100
3) Merged metrics: all of the above

Each figure is a ranker x metric grid:
- rows = metrics
- cols = rankers
- bars = policies (grouped per iteration)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot grouped average-per-iteration metrics across topics.")
    ...
    parser.add_argument(
        "--llm-metrics",
        default="CumRecall,CumPrecision,Jaccard",
        help="Comma-separated LLM-facing metrics for figure 1.",
    )
    parser.add_argument(
        "--retriever-metrics",
        default="CumRecall@100,CumPrecision@100",
        help="Comma-separated retriever-facing metrics for figure 2.",
    )
    parser.add_argument(
        "--ranker-order",
        default="bm25,splade",
    ...
    parser.add_argument(
        "--topicset",
        default=None,
        help="Global ir_datasets topicset. If omitted, inferred from logs.",
    )
    parser.add_argument("--y-max", type=float, default=0.0, help="Max y-axis value (0 = auto per subplot).")
    ...
    parser.add_argument("--output-dir", required=True, help="Directory to save the 3 output figures.")
    parser.add_argument(
        "--output-prefix",
        default="avg_metric_groups",
    ...
    parser.add_argument("--ranking-model", default=None, help="Global ranking model for offline top-100 reranking.")
    parser.add_argument("--index-name", default=None, help="Global OpenSearch index name for offline reranking.")
...
def resolve_per_log_values(values: Optional[List[str]], count: int, flag_name: str) -> List[Optional[str]]:
    if values is None:
        return [None] * count
    if len(values) != count:
        raise SystemExit(f"{flag_name} count ({len(values)}) must match --log count ({count}) when provided.")
...
def build_log_configs(args: argparse.Namespace) -> List[LogRunConfig]:
    ...
    resolved_rankers = [log_rankers[i] or args.ranking_model or inferred_meta[i][1] for i in range(num_logs)]
    ...
    if args.index_map:
        for raw in args.index_map:
            ...
            if len(parts) != 3:
                raise SystemExit(f"Invalid --index-map '{raw}'. Expected '<topicset>|<ranker>|<index_name>'.")
            topicset_key, ranker_key, idx_name = parts
            if not topicset_key or not ranker_key or not idx_name:
                raise SystemExit(f"Invalid --index-map '{raw}'. None of topicset/ranker/index can be empty.")
...
def load_qrel_sets(topicset_name: str) -> Dict[str, set[str]]:
    if ir_datasets is None:
        raise SystemExit("ir_datasets is required to compute CumPrecision@100 offline.")
        ...
        for line in handle:
            ...
            for metric_key in metrics:
                ...
                try:
                    ...
                    continue
                per_metric_topics[metric_key][topic].append((it, value_f))
...
def plot_metric_grid(
    metrics: List[str],
    ranker_order: List[str],
    policy_order: List[str],
    series_map: Dict[str, Dict[str, Dict[str, Tuple[List[int], List[float | None]]]]],
    datasets: set[str],
    args: argparse.Namespace,
    output_path: Path,
    title_prefix: str,
...
def main() -> None:
    ...
    llm_metrics = [m.strip() for m in args.llm_metrics.split(",") if m.strip()]
    retriever_metrics = [m.strip() for m in args.retriever_metrics.split(",") if m.strip()]
    if not llm_metrics:
        raise SystemExit("No llm metrics specified.")
    if not retriever_metrics:
        raise SystemExit("No retriever metrics specified.")

    all_metrics = dedupe_keep_order(llm_metrics + retriever_metrics)
    allowed = {"CumRecall", "CumRecall@100", "Jaccard", "CumPrecision", "CumPrecision@100"}
    unsupported = [m for m in all_metrics if m not in allowed]
    if unsupported:
        raise SystemExit(f"Unsupported metrics: {unsupported}")
    ...
    for log_cfg in log_configs:
        ...
        # Needed for offline CumPrecision@100 fallback.
        if "CumPrecision@100" in all_metrics and log_cfg.topicset not in qrels_cache:
        ...
        ranker, dataset, per_metric_topics = load_log_data(
            ...
            log_cfg=log_cfg,
            metrics=all_metrics,
            qrels_by_topic=qrels_cache.get(log_cfg.topicset, {}),
        )
    ...
    series_map: Dict[str, Dict[str, Dict[str, Tuple[List[int], List[float | None]]]]] = defaultdict(
    ...
    llm_path = output_dir / f"{args.output_prefix}_llm_metrics.png"
    retriever_path = output_dir / f"{args.output_prefix}_retriever_metrics.png"
    merged_path = output_dir / f"{args.output_prefix}_all_metrics.png"

    plot_metric_grid(
        metrics=llm_metrics,
        ranker_order=ranker_order,
        policy_order=policy_order,
        series_map=series_map,
        datasets=datasets,
        args=args,
        output_path=llm_path,
        title_prefix="Average per-iteration LLM-facing metrics across topics",
    )
    plot_metric_grid(
        ...
        datasets=datasets,
        args=args,
        output_path=retriever_path,
        title_prefix="Average per-iteration retriever-facing metrics across topics",
    )
    plot_metric_grid(
        metrics=all_metrics,
        ranker_order=ranker_order,
        ...
        title_prefix="Average per-iteration merged metrics across topics",
    )
